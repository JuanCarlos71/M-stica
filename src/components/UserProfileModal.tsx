...
import { signInWithGoogle, logoutUser, type FirebaseUser } from '../lib/firebase';
...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      alert('No se pudo iniciar sesión con Google. Por favor intenta nuevamente.');
    } finally {
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              <div>
                ...
                <span className="text-[11px] text-gray-300 block">
                  Inicia sesión para guardar tu carta y lecturas en la nube.
                </span>
              </div>
              <button
                onClick={handleGoogleSignIn}
                disabled={isLoggingIn}
                className="w-full sm:w-auto px-4 py-2.5 bg-white text-gray-900 hover:bg-gray-100 font-bold rounded-xl text-[11px] flex items-center justify-center gap-2 shadow-md transition-all cursor-pointer shrink-0"
              >
